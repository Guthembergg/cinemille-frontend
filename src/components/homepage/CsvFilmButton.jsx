...
function ModaleCsvFilm(props) {
  ...
  const PostFetch = async (file) => {
    setLoading(true);
    try {
      const response = await fetch(`http://localhost:8080/film/upload`, {
        method: "POST",
        body: file,
  ...
  return (
    <>
      ...
      >
        Aggiungi film tramite csv
      </Button>

      <Modal show={show} onHide={handleClose} backdrop="static">
        <Modal.Header closeButton>
          <Modal.Title className="ms-auto purple">
            Aggiungi un film tramite csv
          </Modal.Title>
        </Modal.Header>
